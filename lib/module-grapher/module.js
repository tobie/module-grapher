...
function Module(ident, options) {
  ...
  if (!ident.isTopLevel()) {
    var err = new Error('Cannot instantiate Module from unresolved identifier: "' + ident + '".');
    err.errno = main.EUNRES;
    err.module = this;
...
(function(p) {
  ...
  function resolveSrc(pathResolver, callback) {
    var self = this;
    ...
    pathResolver.resolve(this, function(err, p) {
      if (err) {
        self.missing = true;
        self.src = '';
        callback(err);
