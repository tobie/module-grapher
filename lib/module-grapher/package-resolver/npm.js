...
function resolve(module, callback) {
  if (module.packageDescriptor) {
    return new ModuleResolver(module).resolve(callback);
  } else if (module.identifier.isTopLevel() && module.identifier.terms.length > 1) {
    return new ExposedModuleResolver(module).resolve(callback);
  } else {
    return new PackageResolver(module).resolve(callback);
  }
}
