    ...
    if ((type == 'call' || type == 'new') && firstChild[0] == 'name' && firstChild[1] == 'require') {
      ...
      if (firstArg[0] == 'string') {
        ...
        if (identifier.isValid()) {
          ...
          throw new Error('Invalid module identifier: "' + identifier + '" required by "' + module + '".');
        }
      } else if (!exports.allowDynamicModuleIdentifiers) {
        throw new Error('Dynamic module identifier: "' + processor.gen_code(firstArg, true) + '" required by "' + module + '".');
      }
    } else if (ast.length) {
