    ...
    if ((type == 'call' || type == 'new') && firstChild[0] == 'name' && firstChild[1] == 'require') {
      ...
      if (firstArg[0] == 'string') {
        ...
        if (identifier.isValid()) {
          ...
          err = new Error('Invalid module identifier: "' + identifier + '" required by "' + module + '".');
          throw err;
        }
      } else {
        err = new Error('Dynamic module identifier: "' + processor.gen_code(firstArg, true) + '" required by "' + module + '".');
        throw err;
      }
    } else if (ast.length) {
