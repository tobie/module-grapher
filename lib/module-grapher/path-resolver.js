...
(function(p) {
  ...
  function checkPath(currentPath, callback) {
    var self = this;
    _testPath(currentPath, function(err, p) {
      if (err) {
        callback(err);
      } else if (p) {
...
function _removeExtension(p) {
  var ext = path.extname(p);
  return ext ? path.join(path.dirname(p), path.basename(p, ext)) : p;
}

function _testPath(p, callback) {
  var paths = _getPathsWithExtensions(p, true);
  _findFirstPath(paths, callback);
}

function _getPathsWithExtensions(p, includeIndex) {
  var ext, results = [];
  
  p = _removeExtension(p);
...
function _findFirstPath(paths, callback) {
